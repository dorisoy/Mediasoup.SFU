...
#ifdef _WIN32
// Windows version: simplified PID file write (no flock support)
int32_t writePidFile(const std::string& pidFile)
{
    ...
    fprintf(fp, "%d", _getpid());
    fclose(fp);
    SRV_LOGD("write pid: %d to %s success!", _getpid(), pidFile.c_str());
    return 0;
}
#else
int32_t writePidFile(const std::string& pidFile)
...
#ifdef _WIN32
    if (runAsDeamon) {
        SRV_LOGE("daemon mode not supported on Windows, running in foreground");
    }
#else
    if (runAsDeamon) {
        int32_t pid = fork();
