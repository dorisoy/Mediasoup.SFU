@echo off
REM ========================================
REM Quick Setup and Run SFU (Release)
REM ========================================
...
echo.
echo ========================================
echo Mediasoup SFU - Quick Setup (Release)
echo ========================================
echo.
...
if not exist "vc2026\Release\sfu.exe" (
    echo ERROR: sfu.exe not found in vc2026\Release\
    echo.
    ...
    echo    msbuild vc2026\mediasoup-server.sln /p:Configuration=Release
    echo.
    pause
    exit /b 1
)
...
if not exist "vc2026\Release\config.json" (
    echo Generating configuration file...
    call gen_config.bat --release
    if errorlevel 1 (
...
if errorlevel 1 (
    if exist "%OPENSSL_BIN%\%DLL_CRYPTO%" (
        echo Copying OpenSSL DLLs to Release directory...
        copy "%OPENSSL_BIN%\%DLL_CRYPTO%" "vc2026\Release\" >nul
        copy "%OPENSSL_BIN%\%DLL_SSL%" "vc2026\Release\" >nul
        echo Done.
    ) else (
        echo.
...
echo.
echo ========================================
echo Starting SFU Server (Release)
echo ========================================
echo.
